"""Evaluate the four frozen classifiers once on the immutable final holdout."""
...
def validate_freeze(frozen, snapshot):
    frozen = Path(frozen)
    freeze = json.loads((frozen / "freeze.json").read_text(encoding="utf-8"))
    if freeze.get("status") != "frozen_before_holdout_evaluation":
        raise ValueError("Experiment was not frozen before final holdout evaluation")
    ...
    if set(item["model"] for item in freeze.get("models", [])) != set(BACKBONES):
        raise ValueError("Frozen experiment does not contain exactly the four final models")
    holdout_metric_code = freeze.get("holdout_metric_code")
    if holdout_metric_code is None:
        raise ValueError("Frozen experiment does not attest holdout_metric_code")
    frozen_code = (freeze["preprocessing"]["code"], freeze["classification_head_and_pooling"], freeze["data_loading_code"], freeze["metric_code"], holdout_metric_code)
...
def evaluate(frozen, snapshot, output, batch_size=16, device=None, bootstrap_samples=DEFAULT_BOOTSTRAP_SAMPLES, seed=42):
    ...
    freeze = validate_freeze(frozen, snapshot)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate all frozen models once on the final holdout.")
